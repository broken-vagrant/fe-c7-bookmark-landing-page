...
const Button = ({ themeColor, className, ...otherProps }: ComponentPropsWithoutRef<'button'> & { themeColor?: ThemeColor }) => {
  ...
  return (
    <button className={`${colorClasses} md:px-8 md:py-2 px-4 py-2 rounded-md text-[white] shadow-md hover:bg-white border-transparent border-solid border-2 ${className} bg-gra`} {...otherProps}>
    </button>
  )
}
